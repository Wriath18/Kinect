...
const PostPrescription = () => {
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) =>
          setFormData({ ...formData, currentPrescription: downloadURL })
        );
      }
    );
  };
  ...
  const handleSubmit = async (e: { preventDefault: () => void }) => {
    ...
    try {
      ...
      const response = await axios.post(
        `/api/user/upload/${currentUser._id}`,
        formData
      );
  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      <h1 className="text-2xl lg:text-3xl font-semibold text-center my-7">
        Upload Prescription
      </h1>
        ...
        <img
          src={
            currentUser.currentPrescription !== ""
              ? currentUser.currentPrescription
              : `/prescription-abc1234567890987654321.jpg`
          }
          alt="profile"
      ...
      <p className="text-green-500 mt-5">
        {updateSuccess && "It has been updated successfully!"}
      </p>
    </div>
  );
};
